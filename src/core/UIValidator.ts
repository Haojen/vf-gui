...
class DepthBin {
    ...
    public items: UILayout[] = [];
    public length: number = 0;

    public insert(client: UILayout): void {
    ...
    public remove(client: UILayout): void {
...
class DepthQueue {
     ...
     */
    public removeLargestChild(client: UILayout) {
        const hashCode = client.uuid;
     ...
     */
    public removeSmallestChild(client: UILayout) {
        const nestLevel = client.$nestLevel;
        let min = nestLevel;
...
class UIValidator extends PIXI.utils.EventEmitter {
     ...
     * 标记组件属性失效
     */
    public invalidateProperties(target: UILayout): void {
     ...
     * 标记需要重新测量尺寸
     */
    public invalidateSize(target: UILayout): void {
     ...
     * 标记需要重新布局
     */
    public invalidateDisplayList(client: UILayout): void {
     ...
     */
    public validateClient(target: UILayout): void {

        let obj: UILayout|undefined;
        let done = false;
